...
def add_transaction(asset, price, status, message_id, payment_address, recieve_address, value):
...
def manage_new_transaction(message_id, customer_addr, asset_addr, amount, confirmed):
    ...
    if asset == None:
        ...
        add_transaction(None, '0.0', 1, message_id, asset_addr, customer_addr, amount)
        return

    # Check value = price
    if amount != asset.price*1000000:
        # Recived amount does not match price, return payment with error
        add_transaction(asset.id, asset.price, 2, message_id, asset_addr, customer_addr, amount)
        return

    # Call Home Assistant service
    response = call_service(asset.endpoint, asset.token, asset.tech_name, asset.service_data)
    if response.status_code != 200:
        add_transaction(asset.id, asset.price, 3, message_id, asset_addr, customer_addr, amount)
        return
        #print(response.status_text)

    # All is good, forward payment to asset owner
    add_transaction(asset.id, asset.price, 0, message_id, asset_addr, asset.recieve_address, amount)
...
def payout(account):
    ...
    while True:
        ...
        payment = (db.session.query(tbl_settlements)
            .filter(tbl_settlements.sent == False)
            .add_columns(tbl_settlements.id.label('id'), 
            tbl_settlements.recieve_address.label('recieve_address'),
            tbl_settlements.value.label('value'),
            tbl_settlements.sent.label('sent'),
            tbl_settlements.modified_on.label('modified_on'))
            ).one_or_none()
        ...
        if payment is not None:
            ...
            transfer = iw.Transfer(
                ...
                address=payment.recieve_address,
            ...
            # Update db
            payment[0].sent = True
            payment[0].modified_on = func.now()
            db.session.commit()
...
t2 = threading.Thread(target=payout, daemon=True, args=(account,))
